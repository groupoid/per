defmodule Per.Parser.Agda do
  ...
  defp parse_decl(tokens) do
    case tokens do
      ...
      [{:postulate_kw, _, _} | rest] -> parse_axiom_decl(rest)
      [{:data_kw, _, _} | rest] -> parse_data_decl(rest)
      [{:record_kw, _, _} | rest] -> parse_data_decl(rest)
      [{:infix_kw, _, _} | rest] -> parse_infix(rest)
      [{:infixl_kw, _, _} | rest] -> parse_infix(rest)
      [{:infixr_kw, _, _} | rest] -> parse_infix(rest)
  ...
  defp parse_infix(tokens) do
    ...
    rest = Enum.drop_while(tokens, fn t -> elem(t, 0) in [:number, :ident, :operator] end)
    {:ok, {:infix_ignored}, rest}
  end

  defp parse_data_decl(rest) do
    # Simple data/record decl for now, map to axiom/signature
    # Agda data can have parameters and a type, then 'where' and constructors
    case parse_axiom_decl(rest) do
      {:ok, sig, [{:where, _, _} | rest2]} ->
        # Skip constructors for now, just keep the signature
        # In a real impl, we would parse constructors here
        rest3 = Enum.drop_while(rest2, fn t -> elem(t, 0) != :v_right_brace and elem(t, 0) != :v_semicolon end)
        {:ok, sig, rest3}
      res -> res
    end
  end
