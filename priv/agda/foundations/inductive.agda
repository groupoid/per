...
def + (A B: Set) : Set := Σ (x : 𝟐), ind₂ (λ (_ : 𝟐), Set) A B x

def 0-ind (C: 𝟎 → Set) (z: 𝟎) : C z := ind-empty (C z) z

def 1-rec (C: Set) (x: C) : 𝟏 → C := ind₁ (\(_:𝟏), C) x
def 1-ind (C: 𝟏 → Set) (x: C ★) : П (z: 𝟏), C z := ind₁ C x
--def 1-β (C: Set) (c: C): Path C (1-rec C c ★) c := idp C c
--def 1-η (z: 𝟏) : Path 𝟏 ★ z := idp 𝟏 ★

def 2-ind (C: 𝟐 → Set) (x: C 0₂) (y: C 1₂) : П (z: 𝟐), C z := ind₂ C x y
def 2-rec (C: Set) (x y: C) : П (z: bool), C := ind₂ (\(_:𝟐), C) x y

def 2-β₁ (C : 𝟐 → Set) (f : Π (x : 𝟐), C 0₂) (g : Π (y : 𝟐), C 1₂) : PathP (<_>C 0₂) (f 0₂) (ind₂ (λ (x : 𝟐), C x) (f 0₂) (g 1₂) 0₂) := <_> f 0₂
...
def W′ (A : Set) (B : A → Set) : Set := W (x: A), B x
def sup′ (A : Set) (B : A → Set) (x : A) (f : B x → W′ A B) : W′ A B := sup A B x f

def W-ind (A : Set) (B : A → Set) (C : (W (x : A), B x) → Set)
    (g : Π (x : A) (f : B x → (W (x : A), B x)), (Π (b : B x), C (f b)) → C (sup A B x f))
    (w : W (x : A), B x)
  : C w := indᵂ A B C g w

def W-rec (A : Set) (B : A → Set) (C : Set)
...
def W-case (A : Set) (B : A → Set) (C : Set) (g : Π (x : A) (f : B x → (W (x : A), B x)), C)
  : (W (x : A), B x) → C
...
def W-proj₁ (A : Set) (B : A → Set) : (W (x : A), B x) → A
