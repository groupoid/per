...
Pi (O : 𝟏) (A : Set) (B : A → Set) : Set = Π (x : A), B x
Π-lambda (O : 𝟏) (A : Set) (B : A → Set) (b : Pi ★ A B) : Pi ★ A B = λ (x : A), b x
Π-apply (O : 𝟏) (A: Set) (B: A → Set) (f: Pi ★ A B) (a: A) : B a = f a
Π-β (O : 𝟏) (A : Set) (B : A → Set) (a : A) (f : Pi ★ A B) : Path (B a) (Π-apply ★ A B (Π-lambda ★ A B f) a) (f a) = idp (B a) (f a)
Π-η (O : 𝟏) (A : Set) (B : A → Set) (a : A) (f : Pi ★ A B) : Path (Pi ★ A B) f (λ (x : A), f x) = idp (Pi ★ A B) f
Sigma (O : 𝟏) (A : Set) (B : A → Set) : Set = summa (x: A), B x
pair (O : 𝟏) (A: Set) (B: A → Set) (a: A) (b: B a) : Sigma ★ A B = (a, b)
pr₁ (O : 𝟏) (A: Set) (B: A → Set) (x: Sigma ★ A B) : A = x.1
pr₂ (O : 𝟏) (A: Set) (B: A → Set) (x: Sigma ★ A B) : B (pr₁ ★ A B x) = x.2
Σ-β₁ (O : 𝟏) (A : Set) (B : A → Set) (a : A) (b : B a) : Path A a (pr₁ ★ A B (a ,b)) = idp A a
Σ-β₂ (O : 𝟏) (A : Set) (B : A → Set) (a : A) (b : B a) : Path (B a) b (pr₂ ★ A B (a, b)) = idp (B a) b
Σ-η (O : 𝟏) (A : Set) (B : A → Set) (p : Sigma ★ A B) : Path (Sigma ★ A B) p (pr₁ ★ A B p, pr₂ ★ A B p) = idp (Sigma ★ A B) p

Path-1 (O : 𝟏) (A : Set) (x y : A) : Set = PathP (<_> A) x y
idp-1 (O : 𝟏) (A : Set) (x : A) : Path A x x = <_> x
...
J-1 (O : 𝟏) (A : Set) (x : A) (C: D A) (d: C x x (idp A x)) (y: A) (p: Path A x y): C x y p
 = subst (singl A x) (\ (z: singl A x), C x (z.1) (z.2)) (eta A x) (y, p) (contr A x y p) d

J-β (O : 𝟏) (A : Set) (a : A) (C : D A) (d: C a a (idp A a)) : Path (C a a (idp A a)) d (J A a C d a (idp A a))
